...
def test_timed_multiply(caplog):
    with caplog.at_level(logging.DEBUG):
        ...
        assert any("Adding numbers - multiply took" in record.message for record in caplog.records), \
...
async def test_timed_async_add(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check that a log message was emitted
        assert any("async_add took" in record.message for record in caplog.records), \
...
async def test_timed_async_multiply(caplog):
    with caplog.at_level(logging.WARNING):
        ...
        assert result == 42, "async_multiply function should return the product of two numbers"

        # Check that a log message was emitted with the custom message
        assert any("Async multiplying numbers - async_multiply took" in record.message for record in caplog.records), \
...
def test_timed_divide_exception(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert any("divide took" in record.message for record in caplog.records), \
...
async def test_timed_async_divide_exception(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Even if the function raises, the decorator should log the elapsed time
        assert any("async_divide took" in record.message for record in caplog.records), \
...
def test_timed_complex_function(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check that a log message was emitted
        assert any("complex_function took" in record.message for record in caplog.records), \
...
def test_timed_logging_levels(caplog):
    with caplog.at_level(logging.DEBUG):
        ...
        assert any("Adding numbers - multiply took" in record.message for record in debug_logs), \
        ...
        assert any("add took" in record.message for record in info_logs), \
...
def test_timed_multiple_decorators(caplog):
    with caplog.at_level(logging.DEBUG):
        ...
        assert any(first_log in record.message for record in caplog.records), \
            "No log message contains both decorators' messages"

        assert any(second_log in record.message for record in caplog.records), \
...
def test_timed_void_function(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check that a log message was emitted
        assert any("void_function took" in record.message for record in caplog.records), \
...
def test_timed_no_args_function(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check that a log message was emitted
        assert any("no_args_function took" in record.message for record in caplog.records), \
...
def test_timed_keyword_only_function(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check that a log message was emitted
        assert any("keyword_only_function took" in record.message for record in caplog.records), \
...
def test_timed_class_methods(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check that a log message was emitted
        assert any("method_add took" in record.message for record in caplog.records), \
...
async def test_timed_async_class_methods(caplog):
    with caplog.at_level(logging.ERROR):
        ...
        assert any("Method multiply - async_method_multiply took" in record.message for record in error_logs), \
        ...
        assert any(re.match(pattern, record.message) for record in error_logs), \
            "No log message matches the expected pattern."
...
def test_timed_lambda_function(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check that a log message was emitted
        assert any("decorated_lambda took" in record.message for record in caplog.records), \
...
def test_timed_default_mutable_arguments(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert len([record for record in caplog.records if "append_to_list took" in record.message]) == 2, \
...
def test_timed_generator_function(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert any("generator_function took" in record.message for record in caplog.records), \
...
def test_timed_class_constructor(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert any("__init__ took" in record.message for record in caplog.records), \
