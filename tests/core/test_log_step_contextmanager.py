...
import re
import asyncio
from ally_ai_core.ContextManagers import log_step, alog_step
...
def test_log_step_process_data(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert any("Step(Process Data): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the step"
        assert any("Step(Process Data): Executed." in record.message for record in caplog.records), \
...
def test_log_step_process_data_custom(caplog):
    with caplog.at_level(logging.DEBUG):
        ...
        assert result == "test data", "process_data_custom should convert data to lowercase"
    
        # Check start and finish logs at DEBUG level
        assert any("Step(Custom Process): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the custom step"
        assert any("Step(Custom Process): Executed." in record.message for record in caplog.records), \
...
def test_log_step_faulty_process(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert any("Step(Faulty Process): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the faulty step"
        assert any("Step(Faulty Process): Failed! Reason: An error occurred during processing" in record.message for record in caplog.records), \
...
async def test_async_log_step_process_data(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check start and finish logs
        assert any("Step(Async Process Data): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the async step"
        assert any("Step(Async Process Data): Executed." in record.message for record in caplog.records), \
...
async def test_async_log_step_faulty_process(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check start and error logs
        assert any("Step(Async Faulty Process): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the async faulty step"
        assert any("Step(Async Faulty Process): Failed! Reason: Async error occurred" in record.message for record in caplog.records), \
...
def test_log_step_void_function(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check start and finish logs
        assert any("Step(Void Function): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the void step"
        assert any("Step(Void Function): Executed." in record.message for record in caplog.records), \
...
def test_log_step_nested(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert any("Step(Outer Step): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the outer step"
        assert any("Step(Outer Step): Executed." in record.message for record in caplog.records), \
            "No log message for finishing the outer step"
    
        assert any("Step(Inner Step): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the inner step"
        assert any("Step(Inner Step): Executed." in record.message for record in caplog.records), \
...
def test_log_step_empty_name(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert any("Step(): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the step with empty name"
        assert any("Step(): Executed." in record.message for record in caplog.records), \
...
def test_log_step_complex_name(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert any("Step(Complex-Step_123!): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the complex step"
        assert any("Step(Complex-Step_123!): Executed." in record.message for record in caplog.records), \
...
async def test_async_log_step_with_exception(caplog):
    with caplog.at_level(logging.INFO):
        ...
        # Check logs
        assert any("Step(Async Faulty Step): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the async faulty step"
        assert any("Step(Async Faulty Step): Failed! Reason: Async runtime error" in record.message for record in caplog.records), \
            "No log message for failed async step"
...
def test_log_step_multiple_exceptions(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert any("Step(Multiple Exceptions): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the multiple exceptions step"
        assert any("Step(Multiple Exceptions): Failed! Reason: First error" in record.message for record in caplog.records), \
            "No log message for first exception"
        assert any("Step(Multiple Exceptions): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the second exception step"
        assert any("Step(Multiple Exceptions): Failed! Reason: Second error" in record.message for record in caplog.records), \
...
def test_log_step_side_effect(caplog):
    ...
    with caplog.at_level(logging.INFO):
        ...
        # Check log messages
        assert any("Step(Increment Counter): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the counter increment step"
        assert any("Step(Increment Counter): Executed." in record.message for record in caplog.records), \
...
def test_log_step_return_complex_object(caplog):
    with caplog.at_level(logging.INFO):
        ...
        assert result == {"key": "value", "number": 42}, "Function should return the expected complex object"
    
        # Check log messages
        assert any("Step(Return Complex Object): Executing..." in record.message for record in caplog.records), \
            "No log message for starting the return complex object step"
        assert any("Step(Return Complex Object): Executed." in record.message for record in caplog.records), \
